    ...
    debts.forEach(debt => {
        ...
        tableHTML += `
            <tr>
                ...
                <td>
                    <button class="ghost small delete-debt" data-id="${debt.id}">
            ...
            ">
                ...
                <div style="display: flex; justify-content: space-between; align-items: center; margin-top: 15px;">
                    <span style="padding: 4px 12px; border-radius: 15px; font-size: 11px; font-weight: bold; background: ${
                        ...
                        ${debt.remainingAmount === 0 ? 'مسدد بالكامل' : debt.paidAmount === 0 ? 'لم يبدأ السداد' : 'قيد السداد'}
                    </span>
                    <button class="ghost small delete-debt" data-id="${debt.id}">
                        <i class="fas fa-trash"></i>
                        حذف
...
function deleteDebt(debtId) {
    ...
    const debtToDelete = debts[debtIndex];
    
    if (confirm(`هل تريد حذف دين ${debtToDelete.name} بمبلغ ${debtToDelete.totalAmount} ريال؟`)) {
...
// حذف مجموعة الديون (في السجل التفصيلي)
function deleteDebtGroup(index, matchingDebts) {
    if (!matchingDebts || index < 0 || index >= matchingDebts.length) return;
    
    const debtToDelete = matchingDebts[index];
    
    if (confirm(`هل تريد حذف جميع ديون ${debtToDelete.name} بتاريخ ${debtToDelete.date}؟`)) {
        // حذف جميع الديون التي تطابق الاسم والتاريخ والوقت
        debts = debts.filter(debt => 
            !(debt.name === debtToDelete.name && 
              debt.date === debtToDelete.date && 
              debt.timeOfDay === debtToDelete.timeOfDay)
        );
        ...
        updateDashboard();
        updateDebtorSelect();
        showToast('✅ تم حذف مجموعة الديون بنجاح');
        
        // إغلاق سجل الدين التفصيلي
        const debtHistoryCard = document.getElementById('debtHistoryCard');
        if (debtHistoryCard) debtHistoryCard.classList.add('hidden');
    }
}
    ...
    if (debtHistoryCard && debtHistoryList) {
        ...
        // تخزين matchingDebs في خاصية بيانات العنصر
        debtHistoryList.dataset.matchingDebts = JSON.stringify(matchingDebts);
    ...
    matchingDebts.forEach(debt => {
        ...
        if (!debtGroups[key]) {
            debtGroups[key] = {
                ...
                remainingAmount: 0,
                payments: [],
                debts: [] // تخزين كائنات الديون الفردية
            };
        }
        ...
        debtGroups[key].debts.push(debt); // تخزين الدين الفردي
    });
    ...
    Object.values(debtGroups).forEach((debtGroup, index) => {
        const debtColor = getDebtColor(debtGroup.remainingAmount, debtGroup.totalAmount);
        ...
        tableHTML += `
            <tr>
                <td>
                    <div style="display:flex; align-items:center; gap:8px;">
                        <i class="fas fa-user" style="color:var(--primary);"></i>
                        <strong>${debtGroup.name}</strong>
                    </div>
                </td>
                <td><span style="color: var(--warning); font-weight:bold;">${debtGroup.totalAmount.toLocaleString()} ريال</span></td>
                <td><span style="color: var(--info); font-weight:bold;">${debtGroup.dayName} - ${debtGroup.timeOfDay}</span></td>
                <td><span style="color: var(--muted);">${debtGroup.date}</span></td>
                <td><span style="color: var(--success); font-weight:bold;">${debtGroup.paidAmount.toLocaleString()} ريال</span></td>
                <td><span class="${debtColor}" style="font-weight:bold;">${debtGroup.remainingAmount.toLocaleString()} ريال</span></td>
                <td>
                    <button class="ghost small delete-debt-group" data-index="${index}">
                        <i class="fas fa-trash"></i>
                        حذف
                    </button>
                </td>
    ...
    Object.keys(debtors).forEach((name, index) => {
        const debtor = debtors[name];
        const debtColor = getDebtColor(debtor.remaining, debtor.total);
        const progressPercent = debtor.total > 0 ? ((debtor.paid / debtor.total) * 100).toFixed(1) : '0';
            ...
            ">
                <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 15px;">
                    <div>
                        ...
                        <div style="font-size: 12px; color: var(--muted);">
                            ${debtor.debts.length} سجل • ${debtor.date} - ${debtor.timeOfDay}
                        </div>
                    </div>
                    <span class="${debtColor}" style="font-size: 18px; font-weight: bold;">
                        ${debtor.remaining.toLocaleString()} / ${debtor.total.toLocaleString()} ريال
...
function setupViewToggleEvents() {
    ...
    // أحداث حذف مجموعة الدين (في السجل التفصيلي)
    document.querySelectorAll('.delete-debt-group').forEach(btn => {
        eventManager.cleanupElement(btn);
        eventManager.addListener(btn, 'click', function() {
            const index = parseInt(this.getAttribute('data-index'));
            const searchTerm = document.getElementById('searchDebts')?.value || '';
            ...
            if (!isNaN(index) && matchingDebts.length > 0) {
                deleteDebtGroup(index, matchingDebts);
            }
        });
    });
}
...
window.voiceSystemBridge = {
    ...
    // الحصول على الديون النشطة (غير المسددة)
    getActiveDebts: function() {
        return (debts || []).filter(debt => debt.remainingAmount > 0);
    },
    
    // البحث عن متدين
    searchDebtor: function(name) {
        const searchName = (name || '').toLowerCase().trim();
        return (debts || []).filter(debt => 
            debt.name && debt.name.toLowerCase().includes(searchName)
        );
    },
    ...
    // تحديث البيانات للنظام الصوتي
    refreshVoiceData: function() {
        return {
            success: true,
            message: "✅ البيانات محدثة للنظام الصوتي",
            data: {
                debts: debts,
                qats: qats,
    ...
    // استعلام دقيق عن متدين
    queryDebtor: function(name) {
    ...
    deleteDebtorDebts: function(name) {
        console.log('🗑️ جسر الصوت يحاول حذف ديون:', name);
        
        if (!name || name.trim() === '') {
            return {
                success: false,
                message: '❌ الرجاء إدخال اسم المتدين',
                deletedCount: 0
            };
        }
        
        // استخدام مصحح الأسماء إذا كان متاحاً
        if (window.voiceSystem && window.voiceSystem.nameCorrector) {
            name = window.voiceSystem.nameCorrector.correct(name);
            console.log('✅ اسم مصحح للحذف:', name);
        }
        
        const nameLower = name.toLowerCase().trim();
        
        // البحث عن ديون المتدين
        const debtorDebts = debts.filter(debt => 
        ...
        // حساب الإجمالي
        const totalAmount = debtorDebts.reduce((sum, debt) => sum + (debt.totalAmount || 0), 0);
        const remainingAmount = debtorDebts.reduce((sum, debt) => sum + (debt.remainingAmount || 0), 0);
        
        // طلب التأكيد
        const confirmation = confirm(
            `⚠️ تأكيد حذف جميع ديون ${name}\n\n` +
            `• عدد الديون: ${debtorDebts.length}\n` +
        ...
        const deletedCount = originalLength - debts.length;
        ...
        // إشعار بالنجاح
        showToast(`✅ تم حذف ${deletedCount} دين للمتدين ${name}`, 'success');
    ...
    // حذف دين محدد بالتاريخ
    deleteSpecificDebt: function(name, date = null, timeOfDay = null) {
        const nameLower = (name || '').toLowerCase().trim();
        ...
        if (date && timeOfDay) {
            filteredDebts = debts.filter(debt => 
                debt.name && debt.name.toLowerCase().includes(nameLower) &&
                debt.date === date && debt.timeOfDay === timeOfDay
            );
        } else if (date) {
            filteredDebts = debts.filter(debt => 
                debt.name && debt.name.toLowerCase().includes(nameLower) &&
        ...
        if (filteredDebts.length === 0) {
            return {
                success: false,
                message: date ? 
                    `لا توجد ديون للمتدين ${name} بتاريخ ${date}` :
                    `لا توجد ديون للمتدين ${name}`
            };
        }
        
        // طلب التأكيد
        const confirmation = confirm(
            `هل تريد حذف ${filteredDebts.length} دين للمتدين ${name}${date ? ` بتاريخ ${date}` : ''}؟\n` +
        ...
        return {
            success: true,
            message: `✅ تم حذف ${deletedCount} دين للمتدين ${name}${date ? ` بتاريخ ${date}` : ''}`,
            deletedCount: deletedCount
        };
    },
    ...
    // في أي مكان في script3.js يمكنك استخدام:

// 1. نظام التحدث
if (window.tts) {
    window.tts.speak("مرحباً بك في نظام إدارة الديون");
    window.tts.speakAmount(5000); // خمسة ألف ريال
    window.tts.speakNumber(123); // مئة وثلاثة وعشرون
}

// 2. نظام الإشعارات
if (window.voiceNotify) {
    window.voiceNotify.success("تم حفظ الدين بنجاح");
    window.voiceNotify.error("حدث خطأ في الحفظ");
    window.voiceNotify.query("محمد: إجمالي 5000 ريال، مسدد 2000 ريال، باقي 3000 ريال");
    
    // إشعار متقدم مع أزرار
    window.voiceNotify.show({
        title: "تأكيد الحذف",
        message: "هل تريد حذف جميع ديون محمد؟",
        type: "warning",
        duration: "long",
        actions: [
            { name: "confirm", label: "نعم، احذف" },
            { name: "cancel", label: "إلغاء" }
        ],
        onAction: (actionName) => {
...
// جعل دالة الحذف متاحة عالمياً للنظام الصوتي
window.deleteDebtorDebts = function(name) {
    if (window.voiceSystemBridge && window.voiceSystemBridge.deleteDebtorDebts) {
        return window.voiceSystemBridge.deleteDebtorDebts(name);
    }
    return {
        success: false,
        message: '❌ جسر النظام الصوتي غير متاح',
        deletedCount: 0
    };
};

// اختبار الحذف من الكونسول
window.testDelete = function(name = 'محمد') {
    console.log('🧪 اختبار حذف ديون:', name);
    const result = window.deleteDebtorDebts(name);
    console.log('نتيجة الحذف:', result);
    return result;
};
// تحديث النظام الصوتي عند كل تغيير
function updateVoiceSystem() {
    if (window.voiceSystem && typeof window.voiceSystem.updateDebtsData === 'function') {
        try {
            window.voiceSystem.updateDebtsData({
                debts: debts,
                debtors: window.voiceSystemBridge.getDebtors(),
                statistics: window.voiceSystemBridge.getStatistics(),
                timestamp: new Date().toLocaleString('ar-SA')
            });
            console.log('🎤 تم تحديث بيانات النظام الصوتي');
        } catch (error) {
            console.warn('⚠️ فشل تحديث النظام الصوتي:', error);
        }
    }
}

// ربط تحديث النظام الصوتي مع دوال الحفظ
const originalSaveToLocalStorage = saveToLocalStorage;
saveToLocalStorage = function() {
    originalSaveToLocalStorage.call(this);
    setTimeout(updateVoiceSystem, 100);
};

// تحديث النظام الصوتي عند المزامنة
if (typeof syncToFirebaseWithRetry === 'function') {
    const originalSyncToFirebase = syncToFirebaseWithRetry;
    syncToFirebaseWithRetry = async function() {
        await originalSyncToFirebase.call(this);
        setTimeout(updateVoiceSystem, 100);
    };
}

// تحديث النظام الصوتي عند التحميل
setTimeout(() => {
    updateVoiceSystem();
    console.log('🎤 جسر النظام الصوتي جاهز');
}, 2000);
// ============================================
    // بدء التطبيق
